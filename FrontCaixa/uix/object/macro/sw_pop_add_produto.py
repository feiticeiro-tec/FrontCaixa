...
class SWPopAddProduto(MDBoxLayout):
    ...
    async def get_datas(self):
        ...
        data = [[f"{venda['produto__id']}",f"{venda['produto__codigo']}",f"{venda['produto__nome']}",f"{venda['produto__valor']}","1",f"{venda['produto__valor']}"] for index,venda in enumerate(vendas)]
        self.call(data)
        if self._parent:
